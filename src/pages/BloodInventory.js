...
const BloodInventory = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isUpdating) {
        const payload = {
          ...
          addDate: formData.addDate
            ? formData.addDate.format("YYYY-MM-DD")
            : null,
        };
        await axios.patch(
          `/api/BloodUnit/${selectedBlood.bloodUnitId}`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
      ...
      setError(
        `Lưu dữ liệu thất bại! Lỗi: ${
          err.response?.data?.title ||
          err.response?.data?.errors?.Volume ||
          err.message
        }`
      );
    }
  ...
  const handleDelete = async (blood) => {
    ...
    try {
      ...
      await axios.patch(
        `/api/BloodUnit/${blood.bloodUnitId}/update-status`,
      ...
      console.error(
        "Lỗi khi xóa đơn vị máu:",
        err.response?.data || err.message
      );
      setError(
        `Xóa thất bại! Lỗi: ${err.response?.data?.title || err.message}`
      );
    }
  };
  ...
  const totalByType = bloodTypes.reduce((acc, type) => {
    const total = inventory
      .filter(
        (item) =>
          item.bloodTypeName === type.name && item.bloodStatus === "Available"
      )
                  ...
                  <TableBody>
                    {inventory.map((row) => (
                      <TableRow key={row.bloodUnitId}>
                        ...
                        <TableCell>
                          {row.addDate
                            ? new Date(row.addDate).toLocaleDateString()
                            : ""}
                        </TableCell>
                        <TableCell>
                          {row.expiryDate
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <FormControl fullWidth margin="normal">
              ...
              <Select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, bloodTypeId: e.target.value })
                }
              >
                {bloodTypes.map((t) => (
            ...
            {selectedBlood && (
              <TextField
                ...
                onChange={(e) =>
                  setFormData({ ...formData, remainingVolume: e.target.value })
                }
              />
            )}
            <DatePicker
